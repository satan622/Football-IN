	...
	<div class="container">
		<nav class="navbar navbar-light bg-light">
			<select name="searchType" class="col-sm-2"
					...
					<option value="i"
						<c:out value="${cri.searchType == 'i' ? 'selected':'' }" />>ID</option>
					<option value="n"
						<c:out value="${cri.searchType == 'n' ? 'selected':'' }" />>NAME</option>
					<option value="l"
						<c:out value="${cri.searchType == 'l' ? 'selected':'' }" />>LOCATION</option>
					<option value="t"
						<c:out value="${cri.searchType == 't' ? 'selected':'' }" />>TEAM</option>
				</select>
			...
			<table class="table table-hover" style="text-align:center">
				...
				<c:forEach items="${list }" var="userVO">
					<tr>
						...
						<td><a href="/team/readTeam?teamname=${userVO.teamname }"><span class="badge badge-pill badge-primary">${userVO.teamname }</span></a></td>
